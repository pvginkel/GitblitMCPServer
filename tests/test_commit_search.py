...
def test_commit_search_basic(client: GitblitClient, test_repo: str) -> None:
    ...
    # Search for any commit with wildcard
    result = client.search_commits(query="*", repos=[test_repo])

    # May get error if repo not indexed
    if isinstance(result, ErrorResponse):
        assert result.error.code in ("INVALID_REQUEST", "INTERNAL_ERROR")
        pytest.skip(f"Commit search not available for '{test_repo}'")
    ...
    # Validate commit structure if commits exist
    if result.commits:
...
def test_commit_search_with_authors(client: GitblitClient, test_repo: str) -> None:
    ...
    # Use a generic author name that might exist
    result = client.search_commits(query="*", repos=[test_repo], authors=["admin"])
    ...
    for commit in result.commits:
        # Author filter is case-insensitive substring match in most systems
        assert commit.author
...
def test_commit_search_multiple_repos(client: GitblitClient, test_repo: str) -> None:
    """Test commit search across multiple repositories."""
    # Search in test repo and a potentially non-existent repo
    result = client.search_commits(query="*", repos=[test_repo, "another.git"])

    # May get error or results only from existing repos
    if isinstance(result, ErrorResponse):
        pytest.skip("Commit search not available")
